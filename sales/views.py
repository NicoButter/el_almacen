...
def new_sale(request):
    if request.method == 'POST':
        return JsonResponse({
            'success': False,
            'error': 'Use /sales/api/realizar-venta/ para registrar ventas.'
        }, status=405)
...
def realizar_venta(request):
    if request.method == 'POST':
        try:
            ...
            tipo_pago_choices = [choice[0] for choice in Venta.TIPO_PAGO_CHOICES]
            ...
            with transaction.atomic():
                ...
                if es_fiada:
                    ...
                    cuenta_corriente.agregar_saldo(total_compra)
